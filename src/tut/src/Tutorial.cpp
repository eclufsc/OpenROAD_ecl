...
Tutorial::printHello()
{
  ...
  std::vector<std::pair<int,std::string>> ratio; //mapa de razões delta/n.pinos
  ...
  for (auto net: block->getNets()){ //cálculo do delta hpwl-wl de uma net
      ...
      odb::uint length;
      odb::uint layerCount;
      odb::uint *xx; //level Table, dummy

      net->getNetStats(wireCount, viaCount, length, layerCount, xx); //get wirelength
      int len = length;
      std::cout<<"      NetName: "<< netName <<"      NetPinCount: "<< pinCount << std::endl;

      int hpwl=0, stwl=0, dRatio=0;
      ...
      std::cout<<"               WL: "<< len << std::endl; //calculo do wirelength da net
