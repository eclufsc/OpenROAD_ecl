...
std::pair<std::pair<int, int>, std::pair<int, int>> FastRouteCore::computeTotoalUsage()
{
  ...
  for (int i = 0; i < y_grid_; i++) {
    for (int j = 0; j < x_grid_ - 1; j++) {
      ...
      total_usage_h_2d += graph2d_.getUsageH(j, i);//h_edges_[i][j].usage;
  ...
  for (int i = 0; i < y_grid_ - 1; i++) {
    for (int j = 0; j < x_grid_; j++) {
      // 2D edge initialization
      total_usage_v_2d += graph2d_.getUsageV(j, i);
